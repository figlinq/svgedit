...
export const getBsplinePoint = function(t) {
  ...
  const p0 = {
    x: eventContext_.getControllPoint2("x"),
    y: eventContext_.getControllPoint2("y")
  };
  const p1 = {
    x: eventContext_.getControllPoint1("x"),
    y: eventContext_.getControllPoint1("y")
  };
...
export const mouseMoveEvent = function(evt) {
  ...
  switch (eventContext_.getCurrentMode()) {
    ...
    case "resize": {
      ...
      if (
        !eventContext_.getCurrentResizeMode().includes("n") &&
        !eventContext_.getCurrentResizeMode().includes("s")
      ) {
        dy = 0;
      }
      if (
        !eventContext_.getCurrentResizeMode().includes("e") &&
        !eventContext_.getCurrentResizeMode().includes("w")
      ) {
        dx = 0;
      }
    ...
    case "line": {
      ...
      if (evt.shiftKey) {
        xya = snapToAngle(
          eventContext_.getStartX(),
          eventContext_.getStartY(),
          x2,
          y2
        );
        x2 = xya.x;
    ...
    case "image": {
      const square =
        eventContext_.getCurrentMode() === "square" || evt.shiftKey;
      let w = Math.abs(x - eventContext_.getStartX());
      ...
      if (square) {
        w = h = Math.max(w, h);
        newX =
          eventContext_.getStartX() < x
            ? eventContext_.getStartX()
            : eventContext_.getStartX() - w;
        newY =
          eventContext_.getStartY() < y
            ? eventContext_.getStartY()
            : eventContext_.getStartY() - h;
      } else {
    ...
    case "fhellipse":
    case "fhrect": {
      eventContext_.setFreehand(
        "minx",
        Math.min(realX, eventContext_.getFreehand("minx"))
      );
      eventContext_.setFreehand(
        "maxx",
        Math.max(realX, eventContext_.getFreehand("maxx"))
      );
      eventContext_.setFreehand(
        "miny",
        Math.min(realY, eventContext_.getFreehand("miny"))
      );
      eventContext_.setFreehand(
        "maxy",
        Math.max(realY, eventContext_.getFreehand("maxy"))
      );
    }
    // Fallthrough
    case "fhpath": {
      ...
      if (
        eventContext_.getControllPoint2("x") &&
        eventContext_.getControllPoint2("y")
      ) {
        for (i = 0; i < eventContext_.getStepCount() - 1; i++) {
          eventContext_.setParameter(i / eventContext_.getStepCount());
          eventContext_.setNextParameter(
            (i + 1) / eventContext_.getStepCount()
          );
          eventContext_.setbSpline(
            getBsplinePoint(eventContext_.getNextParameter())
          );
          eventContext_.setNextPos({
            x: eventContext_.getbSpline("x"),
            y: eventContext_.getbSpline("y")
          });
          eventContext_.setbSpline(
            getBsplinePoint(eventContext_.getParameter())
          );
          eventContext_.setSumDistance(
            eventContext_.getSumDistance() +
              Math.sqrt(
                (eventContext_.getNextPos("x") -
                  eventContext_.getbSpline("x")) *
                  (eventContext_.getNextPos("x") -
                    eventContext_.getbSpline("x")) +
                  (eventContext_.getNextPos("y") -
                    eventContext_.getbSpline("y")) *
                    (eventContext_.getNextPos("y") -
                      eventContext_.getbSpline("y"))
              )
          );
          if (
            eventContext_.getSumDistance() > eventContext_.getThreSholdDist()
          ) {
            eventContext_.setSumDistance(
              eventContext_.getSumDistance() - eventContext_.getThreSholdDist()
            );
      ...
      eventContext_.setControllPoint2(
        "x",
        eventContext_.getControllPoint1("x")
      );
      eventContext_.setControllPoint2(
        "y",
        eventContext_.getControllPoint1("y")
      );
      eventContext_.setControllPoint1("x", eventContext_.getStart("x"));
      eventContext_.setControllPoint1("y", eventContext_.getStart("y"));
      eventContext_.setStart({
        x: eventContext_.getEnd("x"),
        y: eventContext_.getEnd("y")
      });
      break;
    ...
    case "pathedit": {
      ...
      if (
        eventContext_.getRubberBox() &&
        eventContext_.getRubberBox().getAttribute("display") !== "none"
      ) {
        realX *= currentZoom;
        realY *= currentZoom;
        assignAttributes(
...
export const mouseOutEvent = function() {
  ...
  const { $id } = svgCanvas;
  if (
    eventContext_.getCurrentMode() !== "select" &&
    eventContext_.getStarted()
  ) {
...
export const mouseUpEvent = function(evt) {
  ...
  switch (eventContext_.getCurrentMode()) {
    ...
    case "select":
      if (!isNullish(selectedElements[0])) {
        ...
        if (isNullish(selectedElements[1])) {
          ...
          switch (selected.tagName) {
            ...
            case "foreignObject":
              break;
            default:
              eventContext_.setCurProperties(
                "fill",
                selected.getAttribute("fill")
              );
              eventContext_.setCurProperties(
                "fill_opacity",
        ...
        if (
          realX !== eventContext_.getRStartX() ||
          realY !== eventContext_.getRStartY()
        ) {
    ...
    case "fhellipse":
      if (
        eventContext_.getFreehand("maxx") - eventContext_.getFreehand("minx") >
          0 &&
        eventContext_.getFreehand("maxy") - eventContext_.getFreehand("miny") >
          0
      ) {
        element = svgCanvas.addSVGElementFromJson({
          ...
          attr: {
            cx:
              (eventContext_.getFreehand("minx") +
                eventContext_.getFreehand("maxx")) /
              2,
            cy:
              (eventContext_.getFreehand("miny") +
                eventContext_.getFreehand("maxy")) /
              2,
            rx:
              (eventContext_.getFreehand("maxx") -
                eventContext_.getFreehand("minx")) /
  ...
  if (!keep && !isNullish(element)) {
    ...
    if (
      (eventContext_.getCurrentMode() !== "path" ||
        !eventContext_.getDrawnPath()) &&
      t &&
      t.parentNode &&
...
export const dblClickEvent = function(evt) {
  ...
  if (parent === eventContext_.getCurrentGroup()) {
    return;
  }
