...
(function (window, document) {
  ...
  const cloneSvg = function (sourceSvg) {
    return sourceSvg.cloneNode(true)
  }

  const queueRequest = function (url, callback) {
    requestQueue[url] = requestQueue[url] || []
    requestQueue[url].push(callback)
  }

  const processRequestQueue = function (url) {
  ...
  const loadSvg = function (url, callback) {
  ...
  const injectElement = function (el, evalScripts, pngFallback, callback) {
    ...
    if (!hasSvgSupport) {
      const perElementFallback = el.getAttribute('data-fallback') || el.getAttribute('data-png')
    ...
    loadSvg(imgUrl, function (svg) {
      ...
      const classMerge = [].concat(svg.getAttribute('class') || [], 'injected-svg', el.getAttribute('class') || []).join(' ')
      ...
      const imgData = [].filter.call(el.attributes, function (at) {
      ...
      Object.keys(iriElementsAndProperties).forEach(function (key) {
        ...
        for (let i = 0, elementsLen = elementDefs.length; i < elementsLen; i++) {
          ...
          forEach.call(properties, function (property) {
            ...
            for (let j = 0, referencingElementLen = referencingElements.length; j < referencingElementLen; j++) {
      ...
      if (scriptsToEval.length > 0 && (evalScripts === 'always' || (evalScripts === 'once' && !ranScripts[imgUrl]))) {
        for (let l = 0, scriptsToEvalLen = scriptsToEval.length; l < scriptsToEvalLen; l++) {
  ...
  /**
     ...
     */
  const SVGInjector = function (elements, options, done) {
    // Options & defaults
    options = options || {}
