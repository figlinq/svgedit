      ...
      callback() {
        (function($) {
          ...
          const getSortedElems = (selector, attrName) => {
            return $(
              $(selector)
                .toArray()
                .sort((a, b) => {
                  var aVal = parseInt(a.getAttribute(attrName), 10),
                    bVal = parseInt(b.getAttribute(attrName), 10);
                  return aVal - bVal;
                })
            );
          };
          ...
          const loadFqFigure = () => {
            ...
            if (fid) {
              ...
              var checked = $('#fq-menu-interact-switch').is(':checked');
              if (checked) {
          ...
          const inlineSvgImage = async function(imgId) {
            ...
            await fetch(imgUrl, {
              ...
              .then(text => {
                var replacedObj = $('#fq-svg-container').find('#' + imgId);
                var doc = new DOMParser().parseFromString(text, 'application/xml');
                var newObj = $(doc.documentElement).replaceAll(replacedObj);
        ...
        */
          ...
          const exportImageFromEditor = async () => {
            ...
            var fqElements = $('#fq-svg-container').find('.fq-image, .fq-plot'),
              imageIdArray = [],
              plotIdArray = [],
              attrArray = {},
              curId,
            ...
            if (fqElements.length) {
              ...
              for (const curId in attrArray) {
                var isImage = $('#' + curId).hasClass('fq-image');
          ...
          $(document).on('change', '#fq-file-upload-input', () => {
            var fileName = $('#fq-file-upload-input')[0].files.length
          ...
          $(document).on('click', '#fq-modal-upload-confirm-btn', e => {
            ...
            var world_readable = $('#fq-file-upload-world-readable').val();
            var imageFile = $('#fq-file-upload-input')[0].files[0];
          ...
          $(document).on('change', '#fq-doc-baseunit', e => {
            ...
            var w = $('#fq-doc-setup-width').val();
            var h = $('#fq-doc-setup-height').val();
          ...
          $(document).on('click', '.fq-modal-export-quality', e => {
            ...
            var value = parseInt($('#fq-modal-export-quality-input').val(), 10);
          ...
          $(document).on('focusout', '#fq-modal-export-quality-input', e => {
            var newValue = parseInt($(e.target).val(), 10);
            if (newValue > 100) {
              newValue = 100;
            }
            if (newValue < 10) {
          ...
          $(document).on('click', '#fq-modal-save-confirm-btn', async event => {
            ...
            var fqElements = $('.fq-image, .fq-plot, .fq-figure');
