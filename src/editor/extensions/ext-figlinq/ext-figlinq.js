...
export default {
  ...
  async init({ _importLocale }) {
    ...
    return {
      ...
      callback() {
        var itemListFolder, itemListFile, figlinqUserId, fqModalMode;
        var dataJSON;
        jQuery
          .ajax({
        ...
        jQuery(document).on("click", "#refresh_item_list", () => {
          ...
          itemListFolder = "";
          itemListFile = "";
          const lastFid = jQuery("#fq-modal-content").data("lastFid");
          updateItemList(lastFid, 1);
        });

        jQuery(document).on("click", ".fq-plot-item, .fq-image-item", e => {

          if(fqModalMode === "saveContent"){
            if(jQuery(e.target).hasClass("fq-image-item")){
              jQuery(".fq-plot-item, .fq-image-item").removeClass("is-active");
              jQuery(e.target).addClass("is-active");
              var text = jQuery(e.target).find(".fq-list-item-text").html();
              jQuery("#fq-file-save-name-input").val(text);
              jQuery("#fq-confirm-save-figure").prop("disabled", false);
            }
            return
          }
          ...
          var activeList = [];
          jQuery(".fq-plot-item, .fq-image-item").each(function() {
        ...
        jQuery(document).on("click", ".folder-item", e => {
          ...
          const fname = jQuery(e.target)
            .text()
            .trim();
          jQuery("#fq-modal-content").data("lastFid", fid);
        ...
        jQuery(document).on("click", "#confirm_add_content", e => {          
          ...
          const selector = ".fq-plot-item.is-active, .fq-image-item.is-active";
        ...
        jQuery(document).on("keyup", "#fq-file-save-name-input", () => {
          let val = jQuery("#fq-file-save-name-input").val();
          
          if(val.length) {
            jQuery("#fq-confirm-save-figure").prop("disabled", false);
          } else {
            jQuery("#fq-confirm-save-figure").prop("disabled", true);
          }
        });
        ...
        jQuery(document).on("click", "#save_figlinq_content", () => {
          ...
          jQuery("#fq-modal-content").addClass("is-active");
          jQuery("#fq-confirm-save-figure").prop("disabled", true);
          jQuery("#fq-file-save-name-input").val("");
        ...
        jQuery(document).on("click", "#fq-confirm-save-figure", () => {

          const world_readable = jQuery("#world_readable").val();
          const fname = jQuery("#fq-file-save-name-input").val();
          const lastFid = jQuery("#fq-modal-content").data("lastFid");
          ...
          var csrf_token = "QsqJChOV9ra5FhreGlIQujW72gfE7oU0A9ZNq0TrwkUY7B2WvFphOQMa2gtWVc20";
        ...
        const loadHomeDirContents = () => {
          var lastFid = jQuery("#fq-modal-content").data("lastFid");
